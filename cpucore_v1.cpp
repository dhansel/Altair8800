...
{
  if( !(regS & PS_ZERO) ) 
    { regPC = MEM_READ(regPC) | (MEM_READ(regPC+1) << 8);  TIMER_ADD_CYCLES(10); }
  else
    { regPC += 2; TIMER_ADD_CYCLES(10); }
}
...
void cpu_CNZ(byte opcode)
{
  regPC += 2;
  if( !(regS & PS_ZERO) ) 
    { pushPC(); regPC = MEM_READ(regPC-2) | (MEM_READ(regPC-1) << 8); TIMER_ADD_CYCLES(17); }
  else
    TIMER_ADD_CYCLES(11);
}
